...
def thr_map_update(x1,y1,x2,y2,thresh_map_l):
	...
	#reg_d.plot(ax=ax1, facecolor='grey', edgecolor='red', lw=1)
	mask_d = reg_d.to_mask()
	#thresh_map=thresh_mapper(thresh,image_data1)
	#np.where(mask_d.to_image(thresh_map.shape), thresh_map, 0)
	#thresh_map-mask_d.to_image(thresh_map.shape)
...
def drop2axes(filename, outname):
	...
	for kw in "CTYPE", "CRVAL", "CRPIX", "CDELT", "CUNIT","CROTA":
		try:
			...
			continue
	#for kw in "PC1_1","PC1_2","PC1_3","PC1_4","PC2_1","PC2_2","PC2_3","PC2_4","PC3_1","PC3_2","PC3_3","PC3_4","PC4_1","PC4_2","PC4_3","PC4_4","PV2_1","PV2_2","OBSGEO-X","OBSGEO-Y","OBSGEO-Z":
	...
	if event.key in ['H', 'h'] and toggle_selector.RS.active:
		...
		while xi<x2:
			...
			while yi<y2-2.*r:
				...
				#reg1.plot(ax=ax1, color='green',fill=True)
				mask = reg1.to_mask(mode='center')
				#data = mask.cutout(thresh_map)

				weighted_data_1d = mask.get_values(thresh_map)

				#X, Y = np.mgrid[xi:xi+stepx, yi:yi+stepy]
				#points = np.vstack((X.ravel(), Y.ravel())).T
				#points=points.astype(int)
				
				#plt.scatter(X,Y)
				#print(np.sum(weighted_data_1d))
...
#
	...
	if event.key in ['X', 'x'] and toggle_selector.RS.active:
		...
		#reg = RectanglePixelRegion(PixCoord((x2+x1)/2.,(y2+y1)/2.), width=x2-x1,height=y2-y1)
		#reg.plot(ax=ax1, facecolor='none', edgecolor='blue', lw=1)
		#reg.plot(ax=ax2, facecolor='none', edgecolor='blue', lw=1)
		plt.draw()
	...
	if event.key in ['D', 'd'] and toggle_selector.RS.active:
		...
		#np.where(mask_d.to_image(thresh_map.shape), thresh_map, 0)
		#thresh_map-mask_d.to_image(thresh_map.shape)
		#tr=np.subtract(thresh_map,mask_d.to_image(thresh_map.shape))
		#ax1.imshow(thresh_map,origin='lower')
		#thresh_map=tr
		#global thresh_map
		plt.draw()
		...
		for xi in x_cen:
			for yi in y_cen:
				...
				if np.sum(weighted_data_1d)>limit*stepx*stepy/scale1**2:
					...
					reg2.plot(ax=ax2, facecolor='blue', edgecolor='dimgrey', lw=2)
					reg2.plot(ax=ax2, color='gold',fill=True,alpha=0.5)
					#selector = reg1.as_mpl_selector(ax1,active=True,sync=True,state_modifier_key='clear')
					#rect=artist_instance((xi-0.5*stepx, yi-stepy*0.5), width=stepx,height=stepy,linewidth=1, edgecolor='r', facecolor='none')
					#ax1.add_patch(rect.art())
					#ax2.add_patch(rect.art())
		plt.draw()
		#plt.show()
	if event.key in ['I','i'] and toggle_selector.RS.active:
		...
		for center in celle:
			...
			px, py = w2.wcs_world2pix(wx, wy, 1)
			#reg_sky=RectangleSkyRegion(SkyCoord(wx,wy),width=stepx*u.arcsec,height=stepy*u.arcsec)
			...
			#reg_ds9=RectangleSkyRegion(SkyCoord(wx*u.deg,wy*u.deg),width=stepx*u.arcsec,height=stepy*u.arcsec)
			#print(reg_ds9)
			#reg_ds9.write(grid, format='ds9')
		ran=len(S1)
		grid.close()
		plt.draw()
		...
		#for i in range(0,ran):
			#try:
			if S1[i]<0. or S2[i]<0. or np.isnan(S1[i])==True or np.isfinite(S1[i])==False or np.isfinite(S2[i])==False or np.isnan(S2[i])==True or np.isnan(e_s1[i])==True or np.isnan(e_s2[i])==True:
		...
		#ax3.set_title(r'$k$='+str(round(a[3],1))+r'$\pm$'+str(round(aerr[3],1))+' Spearman: '+str(round(spear[0],2))+' Pearson: '+str(round(per[0],2)))
	...
	if event.key in ['C', 'c'] and toggle_selector.RS.active:
		...
		#ax1.set_xlim(image_data1[0,0],image_data1[-1,0])
		#ax1.set_ylim(image_data1[0,0],image_data1[0,-1])
		plt.draw()
		#if len(celle)!=0:
		celle.clear()
		thresh_map=thresh_mapper(thresh,image_data1)
...
#ax1.tick_params(axis='both', which='both',color='dimgrey')
